...
subroutine kernel(gparam)
    ...
    integer                  :: nthread
    integer                  :: MaxAtom, MaxNb15_Fugaku, cutoff_int2, max_class
    ...
    call kernel_inside(coord_pbc, force, virial,                               &
            coord, trans1, charge, natom, start_atom,                          & 
            atmcls, atmcls_pbc, num_nb15_calc, nb15_calc_list,                 &
            table_grad, nonb_lj12, nonb_lj6,                                   &
            maxcell,ncell,num_atom_domain,nthread,cutoff2, density, &
            MaxAtom, MaxNb15_Fugaku, cutoff_int2, max_class)
...
subroutine kernel_inside(coord_pbc, force, virial,                   &
            coord, trans1, charge, natom, start_atom,                    &
            atmcls, atmcls_pbc, num_nb15_calc, nb15_calc_list,           &
            table_grad, nonb_lj12, nonb_lj6,                             &
            maxcell,ncell,num_atom_domain,nthread,cutoff2, density,      &
    ...
    integer, intent(in)   :: MaxAtom, MaxNb15_Fugaku, cutoff_int2, max_class
    integer, intent(in)      :: maxcell,ncell
    integer, intent(in)      :: num_atom_domain
    integer, intent(in)      :: nthread
    real(wp), intent(in)      :: density, cutoff2
    real(wp), intent(inout) :: force(1:MaxAtom*ncell,1:nxyz,1:1,1:nthread)
    real(wp), intent(inout)  :: virial(1:nxyz,1:nxyz,1:nthread)
    real(wp), intent(inout)  :: coord_pbc(1:MaxAtom*ncell,1:ncq,1:1)
    integer, intent(in)  :: natom(1:ncell)
    integer, intent(in)   :: start_atom(1:ncell)
    integer, intent(in)   :: atmcls(1:MaxAtom,1:ncell)
    integer, intent(in)   :: num_nb15_calc(1:MaxAtom*ncell,1:1)
    integer, intent(in)   :: nb15_calc_list(1:MaxNb15_Fugaku,1:MaxAtom*ncell,1:1)
    integer, intent(inout)   :: atmcls_pbc(1:MaxAtom*ncell)
    real(wp), intent(in)  :: charge(1:MaxAtom,1:ncell)
    real(wp), intent(in)  :: coord(1:nxyz,1:MaxAtom,1:ncell)
    real(wp), intent(in)  :: trans1(1:nxyz,1:MaxAtom,1:ncell)
    real(wp), intent(in)  :: table_grad(1:cutoff_int2*6)
    real(wp), intent(in)  :: nonb_lj12(1:max_class,1:max_class)
    real(wp), intent(in)  :: nonb_lj6(1:max_class,1:max_class)
